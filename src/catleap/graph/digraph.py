...
def draw_digraph(duplication_list: list, graph_name="graphs", min_duplication=2):
  ...
  for duplication_dict in duplication_list:
    ...
    if int(formatted_state.count) > min_duplication:
      G.attr("node", shape="circle", style="filled", weight="200", color="orange" if formatted_state.is_remix_end else "gray")
      G.edge(formatted_state.start,formatted_state.end,label=formatted_state.count, color="red")
