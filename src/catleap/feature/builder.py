      ...
      case "TRANS":
        self.__DATA = {
          "BAS_TO_DEV" : {
            ...
            "DUPLICATIONS": json.load(open(path.BAS_TO_DEV + "duplication.json", "r"))
          },
          "DEV_TO_MAS" : {
            "USERS": json.load(open(path.DEV_TO_MAS + "out-all.json", "r")),
            "DUPLICATIONS" : json.load(open(path.DEV_TO_MAS + "duplication.json", "r"))
          }
        }
        return
      case _:
  ...
  def __extract_trans_features(self, target, cached):
    ...
    if not cached:
      for USER in tqdm(USERS):
        ...
        probablities = self.__calculate_probablity(USER["MILES"], target, is_positive)
      ...
      if target == "BAS_TO_DEV":
        probablities_dict = json.load(open(path.BAS_TO_DEV + "probablities.json", "r"))
      elif target == "DEV_TO_MAS":
        probablities_dict = json.load(open(path.DEV_TO_MAS + "probablities.json", "r"))
