...
def binary_classify(df):
    ...
    skf = StratifiedKFold(
        n_splits=10,             # 10分割交差検証
        random_state=42,         # 乱数の設定
        shuffle=True             # データを選択方法はランダム
    )
...
if __name__ == "__main__":
    ...
    for path in ['./exp_count/', './exp_binary/']:
        ...
        for ctscore in [8, 15]:
            ...
            # ファイル出力
            class_results.to_csv(path + '[RESULT]class_CTScore-{}.csv'.format(ctscore), index=True)
    ...
    with open('./prediction_result.md', 'w') as f:
        ...
        for path in ['./exp_count/', './exp_binary/']:
            ...
            for cs in ['8', '15']:
                ...
                # 精度の平均値を算出
                precision = round(statistics.mean([jl['precision'] for jl in json_load['score']]), 2)
                recall = round(statistics.mean([jl['recall'] for jl in json_load['score']]), 2)
                f1 = round(statistics.mean([jl['f1'] for jl in json_load['score']]), 2)

                f.write('| CTScore-{} | {} | {} | {} |'.format(cs, precision, recall, f1) + '\n')
